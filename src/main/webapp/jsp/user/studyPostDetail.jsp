<%@ page language="java" contentType="text/html; charset=EUC-KR"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="EUC-KR">
<link rel="stylesheet"
	href="https://cdn.inflearn.com/dist/css/npm_tinymce.11812fb7b9e7501ca2a8.css">
<link rel="stylesheet"
	href="https://cdn.inflearn.com/dist/css/MAIN.9d0b26897ffd11f61f81.css">
<link rel="stylesheet"
	href="https://cdn.inflearn.com/dist/css/_post_study_item.8371276e43c7cccade20.css">
<link rel="stylesheet"
	href="https://cdn.inflearn.com/dist/vendor/bulma-accordion.min.css">
<link rel="stylesheet"
	href="https://cdn.inflearn.com/dist/vendor/bulma-switch.min.css">
<link rel="stylesheet"
	href="https://cdn.inflearn.com/dist/vendor/bulma-tooltip.min.css">
<link rel="stylesheet"
	href="https://cdn.inflearn.com/dist/fontawesome/css/all.css">
<title>Insert title here</title>
</head>
<body>

	<br>
	<br>
	<main id="main">
		<section
			class="community-post-detail__section community-post-detail__post"
			data-id="233255">
			<div class="section__sider">
				<div class="sider__space"></div>
				<div class="sider__right">
					<div class="post__features" style="display: none">

						<div class="flex-row feature__status e-status e-hover-toggle"
							data-status="1">

							<button
								class="ac-button is-md is-solid is-primary button-rounded undefined "
								disabled="">모집완료</button>
						</div>

						<div
							class="flex-row feature__recommend e-post-like e-hover-toggle"
							data-id="233255" data-status="" data-cnt="2">

							<button
								class="ac-button is-md is-outlined is-white button-rounded undefined ">
								<svg width="16" height="16" viewBox="0 0 16 16"
									xmlns="http://www.w3.org/2000/svg">
									<path fill="#1B1C1D" fill-rule="evenodd" clip-rule="evenodd"
										d="M4.49095 2.66666C3.10493 2.66666 1.66663 3.92028 1.66663 5.67567C1.66663 7.74725 3.21569 9.64919 4.90742 11.0894C5.73796 11.7965 6.571 12.3653 7.19759 12.7576C7.51037 12.9534 7.7704 13.1045 7.95123 13.2061C7.96818 13.2156 7.98443 13.2247 7.99996 13.2333C8.01549 13.2247 8.03174 13.2156 8.04869 13.2061C8.22952 13.1045 8.48955 12.9534 8.80233 12.7576C9.42892 12.3653 10.262 11.7965 11.0925 11.0894C12.7842 9.64919 14.3333 7.74725 14.3333 5.67567C14.3333 3.92028 12.895 2.66666 11.509 2.66666C10.1054 2.66666 8.9751 3.59266 8.4743 5.09505C8.40624 5.29922 8.21518 5.43693 7.99996 5.43693C7.78474 5.43693 7.59368 5.29922 7.52562 5.09505C7.02482 3.59266 5.89453 2.66666 4.49095 2.66666ZM7.99996 13.8018L8.22836 14.2466C8.08499 14.3202 7.91493 14.3202 7.77156 14.2466L7.99996 13.8018ZM0.666626 5.67567C0.666626 3.368 2.55265 1.66666 4.49095 1.66666C6.01983 1.66666 7.25381 2.48414 7.99996 3.73655C8.74611 2.48414 9.98009 1.66666 11.509 1.66666C13.4473 1.66666 15.3333 3.368 15.3333 5.67567C15.3333 8.22121 13.4657 10.3823 11.7407 11.8509C10.863 12.5982 9.98767 13.1953 9.33301 13.6052C9.00516 13.8104 8.73133 13.9696 8.53847 14.0779C8.44201 14.1321 8.36571 14.1737 8.31292 14.2019C8.28653 14.2161 8.26601 14.2269 8.25177 14.2344L8.2352 14.2431L8.23054 14.2455L8.22914 14.2462C8.22897 14.2463 8.22836 14.2466 7.99996 13.8018C7.77156 14.2466 7.77173 14.2467 7.77156 14.2466L7.76938 14.2455L7.76472 14.2431L7.74815 14.2344C7.73391 14.2269 7.71339 14.2161 7.687 14.2019C7.63421 14.1737 7.55791 14.1321 7.46145 14.0779C7.26858 13.9696 6.99476 13.8104 6.66691 13.6052C6.01225 13.1953 5.13695 12.5982 4.25917 11.8509C2.53423 10.3823 0.666626 8.22121 0.666626 5.67567Z"></path></svg>
								<div class="text features__text">2</div>
							</button>
							<div class="message e-target-hover-toggle hide">2명이 이 글을
								좋아합니다!</div>
						</div>


						<button
							class="ac-button is-md is-outlined is-white button-rounded flex-row features__share e-share ">
							<svg width="16" height="16" viewBox="0 0 16 16"
								xmlns="http://www.w3.org/2000/svg">
								<path fill="#1B1C1D" fill-rule="evenodd" clip-rule="evenodd"
									d="M13.3334 3.66666C13.3334 4.95533 12.2887 6 11 6C10.3257 6 9.7181 5.7139 9.29211 5.25646L5.88379 7.27067C5.95923 7.50012 6.00004 7.74529 6.00004 8C6.00004 8.25469 5.95923 8.49986 5.8838 8.72932L9.29224 10.7434C9.71822 10.286 10.3257 10 11 10C12.2887 10 13.3334 11.0447 13.3334 12.3333C13.3334 13.622 12.2887 14.6667 11 14.6667C9.71138 14.6667 8.66671 13.622 8.66671 12.3333C8.66671 12.0786 8.70752 11.8335 8.78296 11.604L5.37452 9.58992C4.94854 10.0473 4.34103 10.3333 3.66671 10.3333C2.37804 10.3333 1.33337 9.28866 1.33337 8C1.33337 6.71133 2.37804 5.66666 3.66671 5.66666C4.34102 5.66666 4.94853 5.9527 5.37452 6.41007L8.78295 4.39599L8.78321 4.39678C8.70761 4.1671 8.66671 3.92166 8.66671 3.66666C8.66671 2.378 9.71138 1.33333 11 1.33333C12.2887 1.33333 13.3334 2.378 13.3334 3.66666ZM12.3334 3.66666C12.3334 4.40304 11.7364 4.99999 11 4.99999C10.2637 4.99999 9.66671 4.40304 9.66671 3.66666C9.66671 2.93028 10.2637 2.33333 11 2.33333C11.7364 2.33333 12.3334 2.93028 12.3334 3.66666ZM3.66671 9.33333C4.40309 9.33333 5.00004 8.73637 5.00004 8C5.00004 7.26362 4.40309 6.66666 3.66671 6.66666C2.93033 6.66666 2.33337 7.26362 2.33337 8C2.33337 8.73637 2.93033 9.33333 3.66671 9.33333ZM12.3334 12.3333C12.3334 13.0697 11.7364 13.6667 11 13.6667C10.2637 13.6667 9.66671 13.0697 9.66671 12.3333C9.66671 11.5969 10.2637 11 11 11C11.7364 11 12.3334 11.5969 12.3334 12.3333Z"></path></svg>
							<div class="text features__text">공유</div>
						</button>
					</div>
				</div>
			</div>

			<div class="section__content">
				<div class="community-post-info">
					<!-- 제목, 작성자이름, 작성시각, 에디터 -->
					<div class="community-post-info__header">

						<div class="header__title">
							<h1>바닐라 JS와 알고리즘 함께 공부해요</h1>
						</div>
						<div class="header__sub-title">
							<div class="header__sub-title__content">
								<h6 class="user-name">
									<a href="/users/393414/@2now">감자</a>
								</h6>
								<div class="content-date">
									<span class="sub-title sub-title__created-at"> <span
										class="sub-title__title">작성일</span> <span
										class="sub-title__value">21.06.17 19:15</span>
									</span> <span class="sub-title sub-title__view-count"> <span
										class="sub-title__title">조회수</span> <span
										class="sub-title__value">73</span>
									</span>
								</div>
							</div>

						</div>
					</div>
					<div class="community-post-info__content">
						<div class="content__body markdown-body">
							<div class="type-placeholder"
								style="font-size: 15px; letter-spacing: -0.3px; color: #abb0b5; white-space: pre-wrap">
								<p dir="ltr"
									style="line-height: 1.38; margin-top: 0pt; margin-bottom: 0pt">
									<span
										style="font-size: 11.5pt; font-family: Arial; color: #999999; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">
									</span>
								</p>
								<p
									style="color: #abb0b5; font-size: 15px; letter-spacing: -0.3px; white-space: pre-wrap">
									<span style="color: #000000"><span
										style="font-size: 11.5pt; font-family: Arial; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">스터디
											모집글을 아래 양식을 참고해 작성해주세요. <br>
									</span><span
										style="font-size: 11.5pt; font-family: Arial; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">꼼꼼히
											작성하면 멋진 스터디 팀원을 만나실 수 있을거예요. </span><b style="font-weight: normal"
										id="docs-internal-guid-15e0be7d-7fff-b3e1-c7c1-ee564e012eca"></b></span>
								</p>
								<p dir="ltr"
									style="line-height: 1.38; margin-top: 0pt; margin-bottom: 0pt">
									<span
										style="font-size: 11.5pt; font-family: Arial; color: #000000; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">
									</span>
								</p>
								<p dir="ltr"
									style="line-height: 1.38; margin-top: 0pt; margin-bottom: 0pt">
									<span
										style="font-size: 11.5pt; font-family: Arial; color: #000000; background-color: transparent; font-weight: bold; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">[개발
										스터디 모집 내용 예시]</span>
								</p>
								<ul>
									<li dir="ltr" role="presentation" style="line-height: 1.38"><span
										style="font-size: 11.5pt; font-family: Arial; color: #000000; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">스터디
											주제 : 바닐라 자바스크립트와 알고리즘</span></li>
									<li dir="ltr" role="presentation" style="line-height: 1.38"><span
										style="font-size: 11.5pt; font-family: Arial; color: #000000; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">스터디
											목표 : JS 기초공부와 코딩테스트 문제풀이를 통한 알고리즘 공부</span></li>
									<li dir="ltr" role="presentation" style="line-height: 1.38"><span
										style="font-size: 11.5pt; font-family: Arial; color: #000000; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">예상
											스터디 일정(횟수) : 6회</span></li>
									<li dir="ltr" role="presentation" style="line-height: 1.38"><span
										style="font-size: 11.5pt; font-family: Arial; color: #000000; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">예상
											커리큘럼 간략히 : 매 주 스스로 JS를 공부하고 코딩테스트 연습문제 풀이 후 간단한 내용 공유</span>
										<ul></ul></li>
									<li dir="ltr" role="presentation" style="line-height: 1.38"><span
										style="font-size: 11.5pt; font-family: Arial; color: #000000; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">예상
											모집인원 : 4명</span></li>
									<li dir="ltr" role="presentation" style="line-height: 1.38"><span
										style="font-size: 11.5pt; font-family: Arial; color: #000000; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">스터디
											소개와 개설 이유 : 꾸준히 공부하고 경험을 공유하기 위해서</span></li>
									<li dir="ltr" role="presentation" style="line-height: 1.38"><span
										style="font-size: 11.5pt; font-family: Arial; color: #000000; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">스터디
											관련 주의사항 : 열정만 가지고있다면 다 좋습니다.</span></li>
									<li dir="ltr" role="presentation" style="line-height: 1.38"><span
										style="font-size: 11.5pt; font-family: Arial; color: #999999; background-color: transparent; font-weight: bold; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap"><span
											style="color: #000000">스터디에 지원할 수 있는 방법을 남겨주세요. (이메일,
												카카오 오픈채팅방, 구글폼 등.)</span><br></span></li>
								</ul>
								<p dir="ltr"
									style="line-height: 1.38; margin-top: 0pt; margin-bottom: 0pt">
									<span
										style="font-size: 11.5pt; font-family: Arial; color: #999999; background-color: transparent; font-weight: bold; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">
									</span>
								</p>
								<p dir="ltr"
									style="line-height: 1.38; margin-top: 0pt; margin-bottom: 0pt">
									<span
										style="font-size: 11.5pt; font-family: Arial; color: #000000; background-color: transparent; font-weight: bold; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">[프로모션
										이벤트 진행중🎉]</span>
								</p>
								<ul
									style="color: #abb0b5; font-size: 15px; letter-spacing: -0.3px; white-space: pre-wrap">
									<li dir="ltr" role="presentation" style="line-height: 1.38"><span
										style="font-size: 11.5pt; font-family: Arial; color: #000000; background-color: transparent; font-weight: bold; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">스터디
											운영하시고 푸짐한 지원금 받아가세요!(선착순 30팀)&nbsp;</span></li>
									<li dir="ltr" role="presentation" style="line-height: 1.38"><span
										style="font-size: 11.5pt; font-family: Arial; color: #000000; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">이벤트
											기간 : 06.17.목 ~ 07.19.월</span></li>
									<li dir="ltr" role="presentation" style="line-height: 1.38"><span
										style="font-size: 11.5pt; font-family: Arial; color: #000000; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">이벤트
											내용: 이벤트 기간내 스터디 모집글 + 스터디 일지 3회 작성시 전원에게 현금처럼 사용 가능한 2만 포인트
											지원</span></li>
									<li dir="ltr" role="presentation" style="line-height: 1.38"><span
										style="font-size: 11.5pt; font-family: Arial; color: #999999; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap"><span
											style="color: #000000">이벤트 더보기</span> </span><a
										href="https://www.inflearn.com/pages/2021study-promotion"
										target="_blank" rel="noopener noreferrer"><span
											style="font-size: 11.5pt; font-family: Arial; color: #1155cc; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: underline; -webkit-text-decoration-skip: none; text-decoration-skip-ink: none; vertical-align: baseline; white-space: pre-wrap">(클릭)</span></a></li>
									<li dir="ltr" role="presentation" style="line-height: 1.38"><span
										style="color: #000000"><span
											style="font-size: 11.5pt; font-family: Arial; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">스터디
												참여자를 위한 </span><span
											style="font-size: 11.5pt; font-family: Arial; background-color: transparent; font-weight: bold; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap">&lt;인프런
												스터디 가이드💚&gt;</span></span><span
										style="font-size: 11.5pt; font-family: Arial; color: #999999; background-color: transparent; font-weight: 400; font-style: normal; font-variant: normal; text-decoration: none; vertical-align: baseline; white-space: pre-wrap"><span
											style="color: #000000"> 받으러가기</span> <a
											href="https://bit.ly/3xoOTET" target="_blank"
											rel="noopener noreferrer"><span style="color: #3598db">(클릭)</span></a></span></li>
								</ul>
								<p
									style="color: #abb0b5; font-size: 15px; letter-spacing: -0.3px; white-space: pre-wrap">
									<b style="font-weight: normal">&nbsp; &nbsp; &nbsp;&nbsp;</b>
								</p>
							</div>
						</div>

						<div class="content__sub-info">

							<ul class="sub-info__tags">

								<li class="sub-info__tag"><a
									href="https://www.inflearn.com/community/studies?tag=javascript">

										<button
											class="ac-button is-sm is-solid is-gray  ac-tag ac-tag--blue  ">
											<span class="ac-tag__name">javascript</span>
										</button>
								</a></li>
								<li class="sub-info__tag"><a
									href="https://www.inflearn.com/community/studies?tag=알고리즘">

										<button
											class="ac-button is-sm is-solid is-gray  ac-tag ac-tag--blue  ">
											<span class="ac-tag__name">알고리즘</span>
										</button>
								</a></li>
								<li class="sub-info__tag"><a
									href="https://www.inflearn.com/community/studies?tag=코딩테스트">

										<button
											class="ac-button is-sm is-solid is-gray  ac-tag ac-tag--blue  ">
											<span class="ac-tag__name">코딩테스트</span>
										</button>
								</a></li>
							</ul>



						</div>


						<div class="post__features">

							<div class="flex-row feature__status e-status e-hover-toggle"
								data-status="1">

								<button
									class="ac-button is-md is-solid is-primary button-rounded undefined "
									disabled="">모집완료</button>
							</div>

							<div
								class="flex-row feature__recommend e-post-like e-hover-toggle"
								data-id="233255" data-status="" data-cnt="2">

								<button
									class="ac-button is-md is-outlined is-white button-rounded undefined ">
									<svg width="16" height="16" viewBox="0 0 16 16"
										xmlns="http://www.w3.org/2000/svg">
										<path fill="#1B1C1D" fill-rule="evenodd" clip-rule="evenodd"
											d="M4.49095 2.66666C3.10493 2.66666 1.66663 3.92028 1.66663 5.67567C1.66663 7.74725 3.21569 9.64919 4.90742 11.0894C5.73796 11.7965 6.571 12.3653 7.19759 12.7576C7.51037 12.9534 7.7704 13.1045 7.95123 13.2061C7.96818 13.2156 7.98443 13.2247 7.99996 13.2333C8.01549 13.2247 8.03174 13.2156 8.04869 13.2061C8.22952 13.1045 8.48955 12.9534 8.80233 12.7576C9.42892 12.3653 10.262 11.7965 11.0925 11.0894C12.7842 9.64919 14.3333 7.74725 14.3333 5.67567C14.3333 3.92028 12.895 2.66666 11.509 2.66666C10.1054 2.66666 8.9751 3.59266 8.4743 5.09505C8.40624 5.29922 8.21518 5.43693 7.99996 5.43693C7.78474 5.43693 7.59368 5.29922 7.52562 5.09505C7.02482 3.59266 5.89453 2.66666 4.49095 2.66666ZM7.99996 13.8018L8.22836 14.2466C8.08499 14.3202 7.91493 14.3202 7.77156 14.2466L7.99996 13.8018ZM0.666626 5.67567C0.666626 3.368 2.55265 1.66666 4.49095 1.66666C6.01983 1.66666 7.25381 2.48414 7.99996 3.73655C8.74611 2.48414 9.98009 1.66666 11.509 1.66666C13.4473 1.66666 15.3333 3.368 15.3333 5.67567C15.3333 8.22121 13.4657 10.3823 11.7407 11.8509C10.863 12.5982 9.98767 13.1953 9.33301 13.6052C9.00516 13.8104 8.73133 13.9696 8.53847 14.0779C8.44201 14.1321 8.36571 14.1737 8.31292 14.2019C8.28653 14.2161 8.26601 14.2269 8.25177 14.2344L8.2352 14.2431L8.23054 14.2455L8.22914 14.2462C8.22897 14.2463 8.22836 14.2466 7.99996 13.8018C7.77156 14.2466 7.77173 14.2467 7.77156 14.2466L7.76938 14.2455L7.76472 14.2431L7.74815 14.2344C7.73391 14.2269 7.71339 14.2161 7.687 14.2019C7.63421 14.1737 7.55791 14.1321 7.46145 14.0779C7.26858 13.9696 6.99476 13.8104 6.66691 13.6052C6.01225 13.1953 5.13695 12.5982 4.25917 11.8509C2.53423 10.3823 0.666626 8.22121 0.666626 5.67567Z"></path></svg>
									<div class="text features__text">2</div>
								</button>
								<div class="message e-target-hover-toggle hide">2명이 이 글을
									좋아합니다!</div>
							</div>


							<button
								class="ac-button is-md is-outlined is-white button-rounded flex-row features__share e-share ">
								<svg width="16" height="16" viewBox="0 0 16 16"
									xmlns="http://www.w3.org/2000/svg">
									<path fill="#1B1C1D" fill-rule="evenodd" clip-rule="evenodd"
										d="M13.3334 3.66666C13.3334 4.95533 12.2887 6 11 6C10.3257 6 9.7181 5.7139 9.29211 5.25646L5.88379 7.27067C5.95923 7.50012 6.00004 7.74529 6.00004 8C6.00004 8.25469 5.95923 8.49986 5.8838 8.72932L9.29224 10.7434C9.71822 10.286 10.3257 10 11 10C12.2887 10 13.3334 11.0447 13.3334 12.3333C13.3334 13.622 12.2887 14.6667 11 14.6667C9.71138 14.6667 8.66671 13.622 8.66671 12.3333C8.66671 12.0786 8.70752 11.8335 8.78296 11.604L5.37452 9.58992C4.94854 10.0473 4.34103 10.3333 3.66671 10.3333C2.37804 10.3333 1.33337 9.28866 1.33337 8C1.33337 6.71133 2.37804 5.66666 3.66671 5.66666C4.34102 5.66666 4.94853 5.9527 5.37452 6.41007L8.78295 4.39599L8.78321 4.39678C8.70761 4.1671 8.66671 3.92166 8.66671 3.66666C8.66671 2.378 9.71138 1.33333 11 1.33333C12.2887 1.33333 13.3334 2.378 13.3334 3.66666ZM12.3334 3.66666C12.3334 4.40304 11.7364 4.99999 11 4.99999C10.2637 4.99999 9.66671 4.40304 9.66671 3.66666C9.66671 2.93028 10.2637 2.33333 11 2.33333C11.7364 2.33333 12.3334 2.93028 12.3334 3.66666ZM3.66671 9.33333C4.40309 9.33333 5.00004 8.73637 5.00004 8C5.00004 7.26362 4.40309 6.66666 3.66671 6.66666C2.93033 6.66666 2.33337 7.26362 2.33337 8C2.33337 8.73637 2.93033 9.33333 3.66671 9.33333ZM12.3334 12.3333C12.3334 13.0697 11.7364 13.6667 11 13.6667C10.2637 13.6667 9.66671 13.0697 9.66671 12.3333C9.66671 11.5969 10.2637 11 11 11C11.7364 11 12.3334 11.5969 12.3334 12.3333Z"></path></svg>
								<div class="text features__text">공유</div>
							</button>
						</div>
					</div>
				</div>
			</div>
		</section>
		<section
			class="community-post-detail__section community-post-detail__answer">
			<div class="section__content">

				<div class="answer-info">
					<div class="answer-info__header">
						<div class="answer-info__title">
							댓글 <span class="answer-info__title--count">4</span>
						</div>

						<div class="bootstrap-components answer__comment-sort flex-row">

							<div class="sort-item">
								<input type="radio" name="commentSort" value="RECENT"
									id="RECENT" class="e-sort-item"> <label for="RECENT">최신순</label>
							</div>
							<span class="dot">·</span>
							<div class="sort-item">
								<input type="radio" name="commentSort" value="RECOMMEND"
									id="RECOMMEND" class="e-sort-item" checked=""> <label
									for="RECOMMEND">좋아요순</label>
							</div>

						</div>

					</div>
					<div class="community-post-info__content">
						<div class="answer-comment" data-id="new">

							<div class="bootstrap-components comment__input">

								<div class="e-signin">


									<input type="text" placeholder="댓글을 작성해보세요."
										class="form-control form-control-lg " required="">

								</div>

							</div>


							<div class="comment__editor e-comment__editor">
								<div class="markdown-body"></div>
								<div class="bootstrap-components comment__footer flex-row">
									<div class="flex-right mt-2">

										<button type="button"
											class="btn btn-outline-secondary e-post-comment-cancel cancel-btn">

											취소</button>


										<button type="button" class="btn btn-primary e-post-comment">

											등록</button>

									</div>
								</div>
							</div>
						</div>
						<div class="content__body">
							<div
								class="content__body__comment-list e-content__body__comment-list">

								<div class="answer__comment" data-id="116324">
									<a id="116324" class="answer__comment-snap" name="116324"></a>
									<div class="comment__card">
										<div class="comment__header flex-row">
											<img class="comment__user-profile"
												src="https://cdn.inflearn.com/public/main/profile/default_profile.png"
												alt="안진영 프로필">
											<div class="flex-column comment__sub-header">

												<div class="flex-row comment__user-name-container">
													<a href="/users/372707/@" class="comment__user-name">안진영</a>
												</div>
												<span class="comment__updated-at">2021.06.17 오후 9:09</span>
											</div>
										</div>
										<div class="comment__body markdown-body">
											<p>스터디 참여 희망합니다~</p>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</section>
	</main>
</body>
</html>